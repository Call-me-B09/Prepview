...
export default function StepInterview({ 
  ...
  return (
    <div className="flex flex-col gap-8 items-center">
      <h2 className="text-2xl text-white text-center font-medium">{currentQuestion.question}</h2>
      ...
      <button
        ...
        className={`flex items-center gap-2 px-8 py-3 rounded-full text-lg shadow-lg ${isRecording ? "bg-red-600 hover:bg-red-700" : "bg-blue-600 hover:bg-blue-700"} text-white`}
      >
        {isRecording ? <StopCircle size={20} /> : <Mic size={20} />}
        {isRecording ? "Stop Recording" : "Start Recording"}
      </button>
      <button onClick={() => setStep(0)} className="text-sm text-gray-300 underline mt-2">
        Cancel & Back to Menu
      </button>
    </div>
  );
